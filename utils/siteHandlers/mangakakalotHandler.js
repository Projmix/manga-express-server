...
// Function to download an image using Axios with streaming
...
async function scrapeAndDownloadMangakakalotImages(pageUrl) {
  try {
    ...
    const uploadedUrls = await processAndUploadImages(imagePaths, `uploads/mangakakalot/${mangaTitle}/Chapter_${chapterNumber}`);

    return uploadedUrls;
  } catch (error) {
    console.error('Error scraping Mangakakalot images:', error);
    return [];
  }
}

module.exports = scrapeAndDownloadMangakakalotImages;
